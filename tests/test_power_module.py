...
from seat.modules.power_module import (
    ObstacleData,
    PowerFileData,
    PowerCalculatorTools,
    PowerPlotter,
)
...
class TestPowerModule(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dev_not_present = os.path.join(
            script_dir, "data/structured/devices-not-present"
        )
        cls.probabilities = os.path.join(
            script_dir, "data/structured/probabilities/probabilities.csv"
        )
        cls.receptor = os.path.join(
            script_dir, "data/structured/receptor/grain_size_receptor.csv"
        )

        cls.pol_file = os.path.join(
            script_dir, "data/structured/power_files/4x4/rect_4x4.pol"
        )
        cls.power_file = os.path.join(script_dir, "data/structured/power_files/4x4/")
        cls.hydrodynamic_probabilities = os.path.join(
            script_dir, "data/structured/probabilities/hydrodynamic_probabilities.csv"
        )
    ...
    def test_valid_pol_file(self):
        ...
        obstacles = ObstacleData.read_obstacle_polygon_file(self.pol_file)
    ...
    def test_invalid_pol_file_path(self):
        ...
        bad_path = os.path.join(
            script_dir, "non_existent_directory/non_existent_file.pol"
        )
        with self.assertRaises(FileNotFoundError):
            ObstacleData.read_obstacle_polygon_file(bad_path)
    ...
    def test_find_mean_point_of_obstacle_polygon(self):
        ...
        np.testing.assert_array_almost_equal(
            centroids[10:15], expected_centroids, decimal=5
        )
    ...
    def test_plot_test_obstacle_locations(self):
        ...
        self.assertIsInstance(
            fig, plt.Figure, "The output should be a matplotlib figure."
        )
        ...
        expected_num_texts = 2 * len(
            obstacles
        )  # Assuming each obstacle has two text elements
        self.assertEqual(
            num_texts,
            expected_num_texts,
            "Number of text elements in plot is incorrect.",
        )
    ...
    def test_centroid_diffs(self):
        ...
        result_pair = PowerCalculatorTools.centroid_diffs(centroids, test_centroid)
        ...
        self.assertEqual(
            result_pair,
            expected_pair,
            "The identified closest centroid pair is incorrect.",
        )
    ...
    def test_extract_device_location(self):
        ...
        devices_df = ObstacleData.extract_device_location(obstacles, device_index)

        # Check that the result is a DataFrame
        self.assertIsInstance(
            devices_df, pd.DataFrame, "The output should be a pandas DataFrame."
        )
        ...
        self.assertAlmostEqual(
            devices_df.iloc[0]["width"],
            expected_width,
            places=3,
            msg="Device width mismatch.",
        )
        self.assertAlmostEqual(
            devices_df.iloc[0]["height"],
            expected_height,
            places=3,
            msg="Device height mismatch.",
        )
        self.assertEqual(
            devices_df.iloc[0]["lower_left"],
            expected_lower_left,
            "Lower left corner mismatch.",
        )
    ...
    def test_pair_devices(self):
        ...
        devices = ObstacleData.pair_devices(centroids)

        # Verify that the result is a NumPy array
        self.assertIsInstance(
            devices, np.ndarray, "The output should be a NumPy array."
        )
        ...
        for device in devices:
            self.assertTrue(
                device[0] != device[1], "A device cannot pair the same centroid twice."
            )
            self.assertIn(
                device[0],
                centroids[:, 0],
                "First index in pair is not a valid centroid index.",
            )
            self.assertIn(
                device[1],
                centroids[:, 0],
                "Second index in pair is not a valid centroid index.",
            )
    ...
    def test_create_power_heatmap(self):
        ...
        csv_file_path = os.path.join(
            script_dir, "data", "structured", "output", "BC_probability_wPower.csv"
        )
        ...
        fig = PowerPlotter.create_power_heatmap(test_data)
        ...
        self.assertEqual(
            colorbar.get_ylabel(), "MW", "The color bar label should be 'MW'."
        )
        ...
        self.assertEqual(
            ax.get_xlabel(),
            "Longitude [deg]",
            "The x-axis label should be 'Longitude [deg]'.",
        )
        self.assertEqual(
            ax.get_ylabel(),
            "Latitude [deg]",
            "The y-axis label should be 'Latitude [deg]'.",
        )
    ...
    def test_read_power_file(self):
        ...
        expected_power = np.array(
            [125613.836, 113441.609, 346991.812, 289513.188, 209345.75]
        )
        expected_total_power = 9235605.772

        # Call the function
        power, total_power = PowerFileData.read_power_file(real_power_file)

        # Assert the results are as expected based on the real file's content
        np.testing.assert_array_equal(
            power[10:15], expected_power, "Power array does not match expected values."
        )
        self.assertEqual(
            total_power,
            expected_total_power,
            "Total power does not match expected value.",
        )

    def test_sort_data_files_by_runnumber(self):
        bc_data = pd.DataFrame({"run number": [3, 1, 2], "original_order": [2, 0, 1]})
        datafiles = ["file3.out", "file1.out", "file2.out"]
        sorted_files = PowerFileData.sort_data_files_by_runnumber(bc_data, datafiles)
    ...
    def test_sort_bc_data_by_runnumber(self):
        ...
        bc_data = pd.DataFrame({"run number": [3, 1, 2], "value": [30, 10, 20]})
        ...
        expected_sorted_bc_data = pd.DataFrame(
            {
                ...
                "original_order": [
                    ...
                    2,
                    0,
                ],  # This reflects the original order of rows before sorting
            }
        ).reset_index(drop=True)

        # Call the function
        sorted_bc_data = PowerFileData.sort_bc_data_by_runnumber(bc_data)

        # Assert that the sorted DataFrame matches the expected result
        pd.testing.assert_frame_equal(
            sorted_bc_data.reset_index(drop=True),
            expected_sorted_bc_data,
            "bc_data is not sorted correctly by 'run number'.",
        )
    ...
    def test_reset_bc_data_order(self):
        ...
        with self.assertRaises(AttributeError):
            PowerFileData.reset_bc_data_order(bc_data)

    def test_roundup(self):
        # Test cases for the modified function's behavior
        self.assertEqual(
            PowerCalculatorTools.roundup(7, 5), 10
        )  # 7 rounds up to 10 (nearest multiple of 5)
        self.assertEqual(
            PowerCalculatorTools.roundup(3, 5), 5
        )  # 3 rounds up to 5 (nearest multiple of 5)
        self.assertEqual(
            PowerCalculatorTools.roundup(5, 5), 5
        )  # 5 stays 5 (nearest multiple of 5)
        self.assertEqual(
            PowerCalculatorTools.roundup(12, 5), 15
        )  # 12 rounds up to 15 (nearest multiple of 5)
        self.assertEqual(
            PowerCalculatorTools.roundup(-2, 5), 0
        )  # -2 rounds up to 0 (nearest multiple of 5)
        self.assertEqual(
            PowerCalculatorTools.roundup(0, 5), 0
        )  # 0 stays 0 (nearest multiple of 5)
        self.assertEqual(
            PowerCalculatorTools.roundup(1, 5), 5
        )  # 1 rounds up to 5 (nearest multiple of 5)
        self.assertEqual(
            PowerCalculatorTools.roundup(-5, 5), -5
        )  # -5 stays -5 (nearest multiple of 5)
    ...
    def test_calculate_power(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            for file_name in expected_files:
                self.assertTrue(
                    os.path.exists(os.path.join(tmpdirname, file_name)),
